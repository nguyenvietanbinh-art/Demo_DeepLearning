...
class VideoProcessor:
    ...
    def process(
        ...
        while True:
            ...
            for t in tracks:
                if len(t) >= 5:
                    active_ids.add(int(t[4]))

            self.smooth_boxes = {
                k: v for k, v in self.smooth_boxes.items()
                if k in active_ids
            }
            ...
            for track in tracks:
                ...
                if track_id in self.smooth_boxes:

                    px1, py1, px2, py2 = self.smooth_boxes[track_id]
                ...
                self.smooth_boxes[track_id] = (x1, y1, x2, y2)
